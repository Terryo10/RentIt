...
class PayService extends Component{
    ...
    checkPayment = async () => {
        ...
        axios.get(`${baseUrl}${url}${this.state.poll_url}`,requestOptions).then((res) => {
            if (res.status === 200) {
        ...
        return(
            <div>
                ...
                <div className="page-content-wrapper">
                    {this.state.makingPayment ? <div>
                            <h1>Placing Your payment</h1>
                        </div> :this.state.confirmingPayment ?
                        <div className="container">
                        <div className="d-block mb-4">
                            <h6>{this.state.message}</h6>
                            <button  onClick={this.checkPayment} className="btn btn-warning btn-lg w-100">Confirm Payment</button>
                        </div>
                        </div>:this.state.reCheckingStatus ? <div>
                                <h1>Checking Payment</h1>
                            </div> : this.state.cancelledPayment ? <div>
                                <h1>Oops you cancelled your payment</h1>
                            </div> :this.state.paymentDone ? <div>
                                <h1>Payment Done yey view all houses</h1>
                            </div>:
                            <div className="container">
                            <div className="checkout-wrapper-area py-3">
                                <div className="credit-card-info-wrapper"><img className="d-block mb-4"
                                                                               src="/assets/img/bg-img/credit-card.png"
                                                                               alt=""></img>

                                    <div>
                                        <h6 className="text-align-center">Subscription Amount is {this.state.loading ?
                                            <Loading/> : "$" + this.state.price} valid for One Month</h6>
                                    </div>
                                    <div>
                                        <p className="text-align-center">Make Payment here to update your
                                            subscription</p>
                                    </div>
                                    ...
                                    <div className="pay-credit-card-form">
                                        <form onSubmit={this.makePayment} method="POST">
                                            ...
                                            {this.props.paymentLoading ?
                                                ...
                                                <button className="btn btn-warning btn-lg w-100" type="submit">Pay
                                                    Now</button>
                                            }
                                        </form>
                                    </div>
